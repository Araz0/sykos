<div class="sykos-container">
    <div class="sykos-container__name">
        <svg class="sykos-container__name__SY" onclick="namePartClick(this,'/Systemisch')" onmouseover="namePartHover(this)" onmouseout="namePartHoverOut(this)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 227.51 154.97">
            <path d="M46.09,155c-14.53-.85-30.49-4.9-43.43-17.12-3.3-3.12-3.41-6.5-1-10.17.45-.69.78-1.45,1.19-2.16,6.84-11.84,10.08-10.27,18.2-4a53.31,53.31,0,0,0,27.09,11,19.47,19.47,0,0,0,22-20.8c-.46-7.08-5.66-11.5-10.9-14.56-10.35-6-21.48-10.71-32.08-16.34-8.35-4.4-15.85-9.9-20.87-18.31C-3.73,45.79-.4,17.94,20.1,7.32c23.15-12,45.54-8.42,67,4.73,4.82,3,5.18,6.74,2,11.73-7.71,12.3-7.71,12.32-20.6,5.82-7.38-3.73-14.74-7.3-23.42-6.7-7.64.53-13.75,3.65-17.58,10.24-4.17,7.18-1.31,17.74,6.81,22.29,11.74,6.56,24,12.14,36.08,18.19A53,53,0,0,1,89.71,89.34c13.61,18.37,9,45.5-11.38,57.58C68.64,152.66,60.64,154.87,46.09,155Z" />
            <path d="M164.5,56.08c12.06-17,23.58-33.37,35.39-49.55a10.77,10.77,0,0,1,6.75-3.74c4.93-.57,10-.22,15-.16s7.42,3.38,4.8,7.83c-3.64,6.21-7.92,12.07-12.08,18-11.94,16.91-24,33.74-35.88,50.7a11.87,11.87,0,0,0-1.91,6.33c-.14,19.83-.07,39.66-.09,59.49,0,6.8-1.23,8-8.16,8-19.37.09-16,.73-16.11-15.82-.15-17,.08-34-.15-51a15.47,15.47,0,0,0-2.5-8.16C135.92,58.27,122,38.7,108.24,19.07a67.15,67.15,0,0,1-5.15-7.78c-2.61-5.13-.52-8.54,5-8.62,4.5-.06,9,.21,13.49-.08,3.82-.25,6,1.41,8.08,4.43C141,23.26,152.61,39.37,164.5,56.08Z" />
        </svg>
        <svg class="sykos-container__name__K" onclick="namePartClick(this,'/Klarheit')" onmouseover="namePartHover(this)" onmouseout="namePartHoverOut(this)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 112 150.64">
            <path d="M27.62,58.6C37,48.77,46.27,38.84,55.74,29.11,64.81,19.81,74,10.66,83.31,1.57a6.34,6.34,0,0,1,4-1.39,132.88,132.88,0,0,1,15.46.09c2.12.19,5.27,1.53,5.84,3.11.74,2,.06,5.32-1.27,7.18-2.38,3.34-5.7,6-8.64,9C88.46,29.77,78.13,39.93,68,50.27,61.51,56.91,55.32,63.83,48.7,71a39.33,39.33,0,0,0,2.67,3.72c12.11,13.16,24.32,26.22,36.36,39.45q11.51,12.63,22.53,25.71c1.26,1.49,2.17,4.46,1.52,6.06-.77,1.89-3.29,4.12-5.23,4.3a98.73,98.73,0,0,1-18,.07c-5.63-.51-8-6-11.41-9.5-6.61-6.9-12.53-14.46-18.87-21.61C48,107.54,37.64,96,26.73,83.87c-.14,1.79-.33,3.1-.33,4.41v52.48c0,6.88-2.76,9.51-9.58,9.69-15.17.41-16.93-.3-16.82-16.53C.28,92.44.09,50.92.09,9.47.09,1.69,1.66.11,9.25.11h8c7.45,0,9.12,1.67,9.14,9.42V57.94Z" />
        </svg>
        <svg class="sykos-container__name__O" onclick="namePartClick(this,'/Orientierung')" onmouseover="namePartHover(this)" onmouseout="namePartHoverOut(this)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 154.66 154.23">
            <path d="M77.5,0c42-1.35,77.38,33.78,77.16,77.06s-32,78.1-79.38,77.1C31.22,153.2.22,120.73,0,77.06-.23,33.69,34.94-1.43,77.5,0Zm52.14,76.79C130.89,49,105.22,25.31,79.4,24.1,50.24,22.73,25.57,50.37,24.72,74.46c-1.1,31.17,24.23,55.41,52.4,55.38C105.89,129.82,131,105.51,129.64,76.83Z" />
        </svg>
        <svg class="sykos-container__name__S" onclick="namePartClick(this,'/Sicherheit')" onmouseover="namePartHover(this)" onmouseout="namePartHoverOut(this)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 97.32 155.37">
            <path d="M48.17,0c14.42.2,27.06,4.85,39,12.06,4.27,2.58,5.37,6.5,3.09,11a15.68,15.68,0,0,1-1,1.67c-6.44,9.75-7.47,13.19-20.11,5.34-7.33-4.56-15.5-7.42-24.39-6.67-6.82.57-12.88,3-16.55,9.19C23.39,40.75,26.29,50.85,36,56.78a172.9,172.9,0,0,0,22.5,11.13C70.23,73,81.3,79.07,89.56,89.2a33.24,33.24,0,0,1,7.64,19.42c.59,11.16-.88,21.44-8.91,30.37A47.34,47.34,0,0,1,60,154.32c-20.21,3.42-38.43-1.55-54.8-13.83-5.8-4.36-6.52-6.64-3.12-12.7.16-.29.35-.56.51-.86,6.61-11.84,7.91-12.29,18.75-4.75A66.22,66.22,0,0,0,46.93,133c9.2,1.76,22.36-6.56,23.74-15.76,1.23-8.23-3-14.64-9.77-18.67a268,268,0,0,0-25.36-13c-11.1-5.1-21.71-11-28.33-21.52C-6.62,42,2.52,13.47,27.56,4.28,34.07,1.81,41.3,1.35,48.17,0Z" />
        </svg>
    </div>
    <div class="sykos-container__word">
        <div class="sykos-container__word__SY">
            Systemisch
        </div>
        <div class="sykos-container__word__K">
            Klarheit
        </div>
        <div class="sykos-container__word__O">
            Orientierung
        </div>
        <div class="sykos-container__word__S">
            Sicherheit
        </div>
    </div>
</div>